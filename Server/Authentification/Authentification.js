...
module.exports = {
	SignupUser: function(req, res, Queries, SuccessCallback) {
		...
		console.log(
			'[Authentification] Signup requested with POST by ' + req.body.Name
		);
						...
						bcrypt.hash(req.body.Password, 10, function(err, hash) {
							...
							DBUsers.SaveNewUserDB(
								...
								err => {
									...
									console.log(
										'[Authentification] User save in DB'
									);
									//Log on session
		...
		VerifCredentials.CheckLoginCredentials(req.body, (param, err) => {
			...
			DBUsers.CheckUserLogin(
				...
				(err, resp, account) => {
					...
					if (resp) {
						console.log(
							'[Authentification] User successfully logged in'
						);
						//Log on session
